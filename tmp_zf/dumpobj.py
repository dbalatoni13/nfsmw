...
for i,s in enumerate(secs):
    if sname(s[0]) in ('.text','.rodata','.data','.bss','.ctors','.sdata','.sdata2','.dtors'):
        out.append(f'sec {sname(s[0]):10s} size={s[4]:#x}')
for i,s in enumerate(secs):
    if sname(s[0]) == '.symtab':
        strtab = secs[s[5]]
        n = s[4]//16
        for j in range(n):
            ...
            sn = sname(secs[shndx][0]) if 0 < shndx < len(secs) else ('UND' if shndx==0 else str(shndx))
            out.append(f'{name!r:64s} val={val:#x} sz={sz} bind={bind} sec={sn}')
print('\n'.join(out))
