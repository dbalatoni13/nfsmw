...
extern MatrixMultiplier MatrixMultiply;
...
void Skeleton::MirrorPose(float *pose, float *mirrorPose, bool local, const BoneMask *mask) {
        ...
        if (pose == mirrorPose) {
            ...
            for (i = 0; i < n; i++) {
                ...
                if (mask->GetBone(i)) {
                    ...
                    ps = pose + i * DOF;
                    pd = mirrorPose + GetBoneData(i).mLeftRightIdx * DOF;
    ...
    if (!local) {
        ...
        pd = mirrorPose;
        quat = *reinterpret_cast<UMath::Vector4 *>(&pd[4]);
        pd[4] = quat.z;
        pd[5] = quat.w;
        pd[6] = -quat.x;
        pd[7] = -quat.y;
    ...
    if (mask) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (mask->GetBone(i)) {
                ...
                output[i].BuildSQT(poseData[0], poseData[1], poseData[2], poseData[4], poseData[5], poseData[6],
                                   poseData[7], poseData[8], poseData[9], poseData[10]);
