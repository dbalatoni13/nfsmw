// c34ord3 f4: FEMath.h VA PRIMERO. El objetivo emite __ml__12FEQuaternion
// (FEMath.h:218) delante de los cuatro inline de FEList.h, y a FEList.h le
// llega el turno por FEObject.h <- FEButtonMap.h. NO ordenar alfabeticamente.
#include "Speed/Indep/Src/FEng/FEMath.h"
#include "Speed/Indep/Src/FEng/FEButtonMap.h"
// c34ord3 f8: el objetivo emite ~PackageInitStateCB (clase de FEPackage.h,
// destructor SINTETIZADO en FEngine.cpp:200) justo DELANTE de
// FEListBox::GetPCellData, o sea que FEPackage.h se parsea en el fichero 1 y
// antes que FEGameInterface.h. NO ordenar alfabeticamente.
...
void FEButtonMap::ComputeButtonLocation(FEObject *pButton, FEGameInterface *pInterface, FEVector2 &Dest) {
    if (!pInterface || pButton->RenderContext == 0) {
        Dest = *reinterpret_cast<const FEVector2 *>(&pButton->GetObjData()->Pos);
    } else {
        FEMatrix4 Matrix;
        if (!pInterface->GetContextTransform(pButton->RenderContext, Matrix)) {
            ...
            Dest = *reinterpret_cast<const FEVector2 *>(&Temp);
        }
    }
}
