...
class FETag {
  ...
  public:
    ...
    FETag *Next() { // Decl: speed/indep/src/feng/FEPackageReader.h:48
        // Data() es this+4, y el CSE de ese this+4 con los del switch mantiene
        // un puntero extra vivo en todo el bucle.
        return reinterpret_cast<FETag *>(reinterpret_cast<u8 *>(this) + (GetSize() + 4));
    }
};
