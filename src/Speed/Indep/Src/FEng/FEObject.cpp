...
#include "FETypes.h"
#include "FEScript.h"
// c34ord3 f3: el objetivo emite feimage.h(317-318) y FEMultiImage.h(319-320)
// JUNTAS y justo delante de FESlotPool.h(321): o sea que FEImage.h le llega
// por FEMultiImage.h y aqui, antes de FESlotPool.h. NO ordenar: es POSICION.
...
FEObjectDestructorCallback *FEObject::pDestructorCallback = 0; // size: 0x4, address: 0x8041D154, Decl: speed/indep/src/feng/FEObject.cpp:20
      ...
      pData(nullptr)  //
{
    ...
    pSrcResp = static_cast<FEMessageResponse *>(Object.Responses.GetHead());
    while (pSrcResp) {
...
void FEObject::SetupMoveToTracks() {
    ...
    NumTracks = pCurrentScript->TrackCount;
    pTrack = pCurrentScript->pTracks;

    for (u32 i = 0; i < NumTracks; i++) {
        pTrack[i].InterpAction &= 0x7F;

        if (static_cast<u32>(pTrack[i].InterpType - 3) < 2) {
            pDataPtr = pData + pTrack[i].LongOffset * 4;
            ...
            if (pKey) {
                switch (pTrack[i].ParamType) {
                    case 1:
                        pKey->Val = *reinterpret_cast<i32 *>(pDataPtr) - *static_cast<i32 *>(pBase->Val);
                        break;
                    case 2:
                        *reinterpret_cast<float *>(static_cast<unsigned char *>(pKey->Val)) =
                            *reinterpret_cast<float *>(pDataPtr) -
                            *reinterpret_cast<float *>(static_cast<unsigned char *>(pBase->Val));
                        break;
                    case 3:
                        pKey->Val = *reinterpret_cast<FEVector2 *>(pDataPtr) -
                                    *reinterpret_cast<FEVector2 *>(static_cast<unsigned char *>(pBase->Val));
                        break;
                    case 4:
                        pKey->Val = *reinterpret_cast<FEVector3 *>(pDataPtr) -
                                    *reinterpret_cast<FEVector3 *>(static_cast<unsigned char *>(pBase->Val));
                        break;
                    case 5: {
                        FEQuaternion BaseQuat = *static_cast<FEQuaternion *>(pBase->Val);
                        BaseQuat.Conjugate();
                        FEQuaternion qRet = *reinterpret_cast<FEQuaternion *>(pDataPtr) * BaseQuat;
                        pKey->Val = qRet;
                        break;
                    }
                    case 6:
                        pKey->Val = *reinterpret_cast<FEColor *>(pDataPtr) -
                                    *reinterpret_cast<FEColor *>(static_cast<unsigned char *>(pBase->Val));
                        break;
                }
            }
        }
...
void FEObject::SetPosition(const FEVector3 &position, bool bRelative) {
    ...
    if (bRelative) {
        ...
        if (CloseEnoughPosition(position, zero) != true) {
            Flags |= FF_DirtyCode;
        }
    } else {
