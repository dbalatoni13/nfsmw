    ...
    if (pTrack->DeltaKeys.GetNumElements() != 0) {
        switch (pTrack->InterpAction & 0x7f) {
            case 0: {
                ...
                // c36feng: la rama del 1.0f va PRIMERO. GCC 2.9 mete las constantes
                // en el pool en el orden en que expande el RTL, y el objetivo tiene
                // el 1.0f (+0xD0) delante del magico int->double (+0xD8) y del 1e-5.
                // El .text sale byte a byte identico con las dos formas.
...
write_base:
    if (!pKey) {
        i32 *pBase = pBaseKey->GetKeyData()->Val;
    ...
    if (t == 0.0f || t == 1.0f) {
        i32 *pValPtr = pKey->GetKeyData()->Val;
        if (t == 0.0f) {
            pValPtr = pPrevKey->GetKeyData()->Val;
        }
        switch (pTrack->ParamType) {
            case 1: {
        ...
        switch (pTrack->ParamType) {
            case 1:
                FELerpInteger(*reinterpret_cast<i32 *>(&pPrevKey->GetKeyData()->Val), *reinterpret_cast<i32 *>(&pKey->GetKeyData()->Val), t,
                              reinterpret_cast<i32 *>(pBaseValue), reinterpret_cast<i32 *>(pOutDataPtr));
                break;
            case 2:
                FELerpFloat(*reinterpret_cast<float *>(&pPrevKey->GetKeyData()->Val), *reinterpret_cast<float *>(&pKey->GetKeyData()->Val), t,
                            reinterpret_cast<float *>(pBaseValue), reinterpret_cast<float *>(pOutDataPtr));
                break;
            case 3:
                FELerpVector2(*reinterpret_cast<FEVector2 *>(&pPrevKey->GetKeyData()->Val), *reinterpret_cast<FEVector2 *>(&pKey->GetKeyData()->Val), t,
                              reinterpret_cast<FEVector2 *>(pBaseValue), reinterpret_cast<FEVector2 *>(pOutDataPtr));
                break;
            case 4:
                FELerpVector3(*reinterpret_cast<FEVector3 *>(&pPrevKey->GetKeyData()->Val), *reinterpret_cast<FEVector3 *>(&pKey->GetKeyData()->Val), t,
                              reinterpret_cast<FEVector3 *>(pBaseValue), reinterpret_cast<FEVector3 *>(pOutDataPtr));
                break;
            case 5:
                FELerpQuaternion(*reinterpret_cast<FEQuaternion *>(&pPrevKey->GetKeyData()->Val), *reinterpret_cast<FEQuaternion *>(&pKey->GetKeyData()->Val), t,
                                 reinterpret_cast<FEQuaternion *>(pBaseValue), reinterpret_cast<FEQuaternion *>(pOutDataPtr));
                break;
            case 6:
                FELerpColor(*reinterpret_cast<FEColor *>(&pPrevKey->GetKeyData()->Val), *reinterpret_cast<FEColor *>(&pKey->GetKeyData()->Val), t,
...
void FELerpQuaternion(FEQuaternion &q1, FEQuaternion &q2, float t, FEQuaternion *pOffset, FEQuaternion *pDest) {
    ...
    if (Dot < 0.999f) {
        ...
        FEQuaternion r = ((q1 * SinAInvT) + (q * SinAT)) * (1.0f / SinA);
        q.x = r.x;
        q.y = r.y;
        q.z = r.z;
        q.w = r.w;
    } else {
        FEQuaternion r = q1 + (q2 - q1) * t;
