...
#include "FEGroup.h"
// c34ord3 f7: FEAnimImage.h antes que FESimpleImage.h; el objetivo emite
// ~FEAnimImage/Clone (338-339) delante de ~FESimpleImage/Clone (340-341).
...
bool FEPackageReader::ReadObjectChunk() {
    ...
    while (pObjChunk->GetID() != Chunk_FEObject) {
        if (pObjChunk >= pLast) {
            return true;
        }
    ...
    while (pTag < pEnd) {
        switch (pTag->GetID()) {
            ...
            case Tag_FEScriptHeader:
                ...
                pScript->Length = pTag->Getu32(1);
                pScript->Flags = pTag->Getu32(2);
                pScript->SetTrackCount(pTag->Getu32(3));
                break;
            ...
            case Tag_FEScriptTrackOffset:
                pTrack->LongOffset = pTag->Getu16(0);
                break;
                    ...
                    if (!pTrack) {
                        ...
                        for (u32 DestIndex = 0; DestIndex <= pScript->TrackCount; DestIndex++) {
                            if (pSrcTrack && SrcIndex < pScript->TrackCount) {
                                if (pField) {
                                    if (pSrcTrack[SrcIndex].LongOffset >= static_cast<int>(pField->GetOffset()) / 4) {
                                        goto insert_track;
                                    }
                                }
                        ...
                        insert_track:
                            ...
                            pNewArray[DestIndex].Length = pScript->Length;
                            pNewArray[DestIndex].LongOffset = static_cast<int>(pField->GetOffset() >> 2);
            ...
            case Tag_FEScriptTrackInterpType:
                if (pScript) {
                    pTrack->InterpType = static_cast<u8>(pTag->Getu16(0));
                }
                break;
            case Tag_FEScriptTrackInterpAction:
                if (pScript) {
                    pTrack->InterpAction = static_cast<u8>(pTag->Getu16(0));
                }
                break;
            case Tag_FEScriptTrackBaseKey: {
                u32 KeyLongs = pTrack->ParamSize >> 2;
                pTrack->BaseKey.tTime = pTag->Getu32(0);

                for (u32 i = 0; i < KeyLongs; i++) {
                    pTrack->BaseKey.Val.Data[i] = pTag->Getu32(i + 1);
                }
            ...
            case Tag_FEScriptFieldKeyData: {
                ...
                do {
                    ...
                    pKey->tTime = FEngGetu32(*pSrc);
                    pSrc++;
                    Index = 0;
                    while (Index < Count) {
                        pKey->Val.Data[Index] = FEngGetu32(pSrc[Index]);
                        Index++;
                    }
                    if (CurKey != 0) {
...
bool FEPackageReader::ReadMessageTargetListChunk() {
    FETag *pTag;
    FETag *pLast;
    FEChunk *pTargetsChunk;
    u32 CurMsgTarg;

    pTargetsChunk = FindChild(pChunk, Chunk_FEMessageTargetList);
    if (pTargetsChunk) {
        ...
        while (pTag < pLast) {
            switch (pTag->GetID()) {
                ...
                case Tag_FEMsgTarget: {
                    ...
                    u32 *pData = reinterpret_cast<u32 *>(pTag->Data()) + 1;
                    u32 MsgID = pTag->Getu32(0);
                    pPack->pMsgTargets[CurMsgTarg].SetMsgID(MsgID);
...
// c34ord3 f6: el objetivo emite FindReferencedObject DETRAS de
// ReadMessageTargetListChunk, no delante de ReadMessageResponseTags.
// NO volver a subirla.
