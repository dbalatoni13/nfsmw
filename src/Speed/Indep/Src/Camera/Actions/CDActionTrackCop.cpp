...
class CDActionTrackCop : public CameraAI::Action, public IAttachable, public ITrafficCenter {
  public:
    ...
    const Attrib::StringKey &GetName() const override {
        static Attrib::StringKey name("CDActionTrackCop");
        return name;
    }
    ...
    USE_FASTALLOC(CDActionTrackCop)
...
UTL::COM::Factory<CameraAI::Director *, CameraAI::Action, UCrc32>::Prototype
    _CDActionTrackCop("CDActionTrackCop", CDActionTrackCop::Construct);
...
CameraAI::Action *CDActionTrackCop::Construct(CameraAI::Director *director) {
    ...
    if (simable) {
        ...
        return new CDActionTrackCop(director, found);
    }
...
CDActionTrackCop::CDActionTrackCop(CameraAI::Director *director, IPlayer *player)
    ...
    mMover = new ("TrackCopCameraMover", 0) TrackCopCameraMover(director->GetView(), mAnchor, false);
    mMover->ResetState();
    static_cast<TrackCopCameraMover *>(mMover)->SetRenderCarPOV(render_pov);
}
