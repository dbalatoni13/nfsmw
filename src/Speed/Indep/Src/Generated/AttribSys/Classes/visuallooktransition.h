...
struct visuallooktransition : Instance {
    ...
    const UMath::Matrix4 &uves_extreme() const {
        const UMath::Matrix4 *resultptr = reinterpret_cast<const UMath::Matrix4 *>(GetAttributePointer(0x0e629621, 0));
        if (!resultptr) {
    ...
    const UMath::Matrix4 &normal() const {
        const UMath::Matrix4 *resultptr = reinterpret_cast<const UMath::Matrix4 *>(GetAttributePointer(0x0eb3d357, 0));
        if (!resultptr) {
            resultptr = reinterpret_cast<const UMath::Matrix4 *>(DefaultDataArea(sizeof(UMath::Matrix4)));
        }
        return *resultptr;
    ...
    const UMath::Matrix4 &uves_pulse() const {
        const UMath::Matrix4 *resultptr = reinterpret_cast<const UMath::Matrix4 *>(GetAttributePointer(0x334f1e4d, 0));
        if (!resultptr) {
            resultptr = reinterpret_cast<const UMath::Matrix4 *>(DefaultDataArea(sizeof(UMath::Matrix4)));
        }
        return *resultptr;
    ...
    const UMath::Matrix4 &uves() const {
        const UMath::Matrix4 *resultptr = reinterpret_cast<const UMath::Matrix4 *>(GetAttributePointer(0x681bef75, 0));
        if (!resultptr) {
            resultptr = reinterpret_cast<const UMath::Matrix4 *>(DefaultDataArea(sizeof(UMath::Matrix4)));
        }
        return *resultptr;
