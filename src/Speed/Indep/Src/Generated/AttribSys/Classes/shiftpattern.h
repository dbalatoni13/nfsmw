...
struct shiftpattern : Instance {
    ...
    const UMath::Matrix4 &Up_Engage_Curve() const {
        const UMath::Matrix4 *resultptr = reinterpret_cast<const UMath::Matrix4 *>(GetAttributePointer(0x68da6275, 0));
        if (!resultptr) {
    ...
    const UMath::Matrix4 &Up_DisengageFall_Curve(unsigned int index) const {
        const UMath::Matrix4 *resultptr = reinterpret_cast<const UMath::Matrix4 *>(GetAttributePointer(0xf040e6b0, index));
        if (!resultptr) {
            resultptr = reinterpret_cast<const UMath::Matrix4 *>(DefaultDataArea(sizeof(UMath::Matrix4)));
        }
        return *resultptr;
