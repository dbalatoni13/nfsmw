        ...
        if (!mBusy) {
            ...
            if (active.empty()) {
                ...
                if (caller) {
                    ...
                    if ((Manager::GetHistory().GetCount(static_cast<SPCHType_1_EventID>(0xa1)) == 0) && !ai->IsHighIntensity()) {
                        ...
                        this->mBusy++;
                        return;
                    } else if ((Manager::GetHistory().GetCount(static_cast<SPCHType_1_EventID>(0x43)) == 0) ||
                        ...
                        this->mBusy++;
                        return;
                    } else if ((Manager::GetHistory().GetCount(static_cast<SPCHType_1_EventID>(0x49)) == 0) && !ai->IsHighIntensity()) {
...
void StrategyFlow::Waiting() {
    ...
    // Los casos salen con `break` y el `return` va DETRAS del switch (no `goto cleanup`
    // en cada caso): eso es lo que funde los dos ChangeStateTo(kOutcome) en uno solo.
    ...
    // Este `return` parece redundante con el `cleanup:` de debajo, pero no lo es para
    // el cross-jumping: sin el, este bloque cae al epilogo y se lleva la etiqueta del
    // despacho generico de ChangeStateTo, y salen 16 B de mas. Con el, el objetivo casa.
    return;
cleanup:;
    return;
no_backup:
