...
static const float MIN_RPM_FOR_SHIFT_FX = 3000.0f;
static const float SECOND_GEAR_LFO_SCALE = 0.85f;
static const float THIRD_GEAR_LFO_SCALE = 0.7f;
static const float FOURTH_GEAR_LFO_SCALE = 0.55f;
static const float FIFTH_GEAR_LFO_SCALE = 0.3f;
...
float DOWN_SHIFTING_REV_PERCENT = 100.0f;
int DOWN_SHIFTING_REV_RAMP_TIME = 215;
SndBase::TypeInfo SFXCTL_Shifting::s_TypeInfo = {(int)0x00020020, "SFXCTL_Shifting", &SFXCTL::s_TypeInfo, SFXCTL_Shifting::CreateObject};
...
void SFXCTL_Shifting::UpdateGearShiftState(float t) {
    if (SndBase::m_fRunningTime > this->tShiftDelay &&
        this->m_bPendingNeedShiftSound) {
        this->m_bNeed_ShiftGearSnd = true;
        this->m_bPendingNeedShiftSound = false;
    }

    this->m_bShouldBeWhining = static_cast<int>(this->GetCurGear()) == 0;
    ...
    case SHFT_DOWN_DISENGAGE: {
        ...
        if (this->m_pEngineCtl->m_pPhysicsCtl->m_CurGear < SECOND_GEAR) {
            Length = static_cast<int>(static_cast<float>(Length) * THIRD_GEAR_LFO_SCALE);
        }
