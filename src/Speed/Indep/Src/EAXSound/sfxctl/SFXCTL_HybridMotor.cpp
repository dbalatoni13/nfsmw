...
static const int LPFSmoothing = 6000;
static const float TRQ_THRESHOLD = 35.0f;

SndBase::TypeInfo SFXCTL_HybridMotor::s_TypeInfo = {(int)0x00020050, "SFXCTL_HybridMotor", &SFXCTL::s_TypeInfo, SFXCTL_HybridMotor::CreateObject};
...
void SFXCTL_HybridMotor::UpdateParams(float t) {
    ...
    this->m_GinsuScaledRPM = (bClamp(AemsRPM, 1000.0f, 10000.0f) - 1000.0f) * (1.0f / 9000.0f) * (this->m_pEAXCar->GetAttributes().MaxRPM() - this->m_pEAXCar->GetAttributes().MinRPM()) + this->m_pEAXCar->GetAttributes().MinRPM();
...
void SFXCTL_HybridMotor::UpdateSingleMixEng(float t) {
    ...
    DeltaRPM = this->m_AvgDeltaRPM.GetValue();
    ...
    this->PercentOfDecelThreshold =
        1.0f - (this->m_pEAXCar->GetAttributes().DecelDeltaRPMThreshold() - bAbs(DeltaRPM + 10.0f)) /
    ...
    AccelMix.DecelGinsu = 0.0f;
    int lpc = static_cast<int>(this->m_pEAXCar->GetAttributes().GINSU_LowPassCutoff());
    DecelMix.Cutoff = bMin(this->m_pEngineCtl->m_DistanceFltr, lpc);
    ...
    this->m_GinsuLPFVal = static_cast<float>(
        25000 - NFSMixShape::GetCurveOutput(SHAPE_UP_ONE_MIN_EQPWR_SQ,
...
void SFXCTL_HybridMotor::UpdateDualMixEng(float t) {
    ...
    DecelMix.Aems = (1.0f - this->DecelCrossfadeMix.GetValue(this->m_GinsuScaledRPM)) +
                    this->DecelCrossfadeMix.GetValue(this->m_GinsuScaledRPM) * DecelMix.Aems;
    ...
    this->m_GinsuLPFVal = static_cast<float>(NFSMixShape::GetCurveOutput(
        SHAPE_UP_ONE_MIN_EQPWR_SQ, this->m_EngineMix.Cutoff, true));
...
void SFXCTL_HybridMotor::UpdateMixerOutputs() {
    ...
    if (speedMph > 30.0f && bAbs(this->m_AvgDeltaRPM.GetValue()) < 30.0f) {
