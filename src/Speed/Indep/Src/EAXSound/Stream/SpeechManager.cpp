...
extern int SPCH_Init(int (*request)(SPCHType_SampleRequestData *), unsigned int seed, int rate);
...
static unsigned int gSpeechSeed = 0;
int VALIDATE_SED_GENERATE = 0;
int SPEECH_DISPLAY_HISTORY = 0;
...
Timer Manager::mSampleReqTimer(0);
...
ScheduledSpeechEvent *Manager::ScheduleSpeechPartII(unsigned int size, void *data, Csis::InterfaceId &iid, Csis::FunctionHandle &fh, EAXCharacter *actor) {
    ...
    if ((IsSpeechEnabled == 0) || (m_speechMode == SPEECH_SPLITSCREEN_MODE) || (g_pEAXSound == 0) ||
        ...
        // El DWARF declara AQUI otro par eventkey/eventID que oculta a los de
        // funcion: el interior vive en r4 (efimero) y por eso el original hace
        // `mr r4,r3` en vez de guardar el valor en una callee-saved.
        ...
        if (eventID == kSPCH1_EventID_CellCall) {
            MNotifyCellCallStarted().Send(UCrc32(0x20d60dbf));
    ...
    for (int i = 0; i < 4; ++i) {
        for (SchedSpchEvents::iterator iter = mEvents[i].begin(); iter != mEvents[i].end(); ++iter) {
            ...
            if (this_event->ID == eventID) {
                ...
                // El objetivo salta a `li r9,1` con `beq` y a la salida con `b`:
                // con el ternario sale al reves. Es un `||` de dos `&&` (el
                // segundo test de `actor` lo borra -fcse-follow-jumps).
                if ((actor == 0 && this_event->actor == 0) || (actor != 0 && this_event->actor != 0 && this_event->actor != actor)) {
                    obj_updated = true;
                }

                if (obj_updated) {
                    // El mapa de lineas da 174 al `lwz WorldTimer` y 178 al
                    // `stw r28,0xc(r3)`: entry_time va ANTES que actor.
    ...
    event->actor = actor;
    event->ID = eventID;
    // El mapa de lineas mete el test, las DOS lecturas de priority() y el UNICO
    // `stb` en la misma linea 236: es un ternario, no un if/else. Con el
    // ternario GCC materializa `&event->priority` (addi r30,r9,0x3b) ANTES de
    // evaluar la condicion y las dos ramas comparten el store.
...
void Manager::Init2() {
    ...
    switch (m_speechMode) {
        case SPEECH_GAME_MODE:
            ...
            if (m_SpeechModule[COPSPEECH_MODULE] && !m_gameSpeechInitted) {
                return;
            }

            if (m_SpeechModule[NISSFX_MODULE] && !m_NISAudioInitted) {
                return;
            }
            break;
    ...
    int bankPtrMemSize = SPCH_GetBankPtrMemSize(m_numberSpeechBanks);

    if (m_speechMode == SPEECH_GAME_MODE) {
        m_SPEECH_bankPtrMem = gAudioMemoryManager.AllocateMemoryChar(bankPtrMemSize, "AUD:Speech Bank Ptrs", false);
    } else {
        m_SPEECH_bankPtrMem = gAudioMemoryManager.AllocateMemoryChar(bankPtrMemSize, "AUD:Speech Bank Ptrs", false);
    }
...
int Manager::LoadSpeechBank(CLUMP_IDX_FILE *index, int &type, int &number, SPEECH_BANK *sb) {
...
int Manager::GetTicker() {
    float secs = static_cast<float>(RealTimeFrames) * (1.0f / 60.0f);
...
// TRACKSTREAMER_BACKLOG_THRESH es GLOBAL (asi la nombra el objetivo, sin
// mangler de namespace), asi que hay que cerrar y reabrir Speech para definirla
// en su sitio de .data.
} // namespace Speech

float TRACKSTREAMER_BACKLOG_THRESH = 1.0f;

namespace Speech {

char *GameSpeech::m_tempCharPtr = 0;
CLUMP_IDX_FILE *GameSpeech::m_clumpIdx = 0;
...
GameSpeech::GameSpeech()
    : mLoadState(), //
      ...
      m_Chirper(0), //
      m_ChirpParams(0, 0x7FFF, 0x1000, 0, 0, 0) {
    mLoadState.clear();
...
void GameSpeech::Init(int channel) {
    ...
    if (nfilesize > 0) {
        *tempCharPtr = static_cast<char *>(bMalloc(nfilesize, "AUD_tmp: Game speech index", 0, 0x1040));
    } else {
        *tempCharPtr = 0;
    }
...
void GameSpeech::LoadingCallback(int param, int) {
    ...
    if (!obj->mLoadState.empty()) {
        ...
        case 0:
            ...
            break;
        case 1: {
            m_clumpIdx = reinterpret_cast<CLUMP_IDX_FILE *>(m_tempCharPtr);
...
void GameSpeech::Update() {
    ...
    if (ss.outstandingrequests > 0) {
        ...
        if (m_pendingList.size() != 0) {
            SpeechSampleVec::iterator i = std::remove_if(m_pendingList.begin(), m_pendingList.end(), Unlocked);
...
} // namespace Speech   // r61-resto: el #include tiene que caer FUERA de
...
CLUMP_IDX_FILE *SED_NISSFX::m_clumpIdx = 0;
...
void SED_NISSFX::Init(int channel) {
    if (IsNISAudioEnabled) {
        ...
        char **tempCharPtr = &m_tempCharPtr;
        if (nfilesize > 0) {
            *tempCharPtr = static_cast<char *>(bMalloc(nfilesize, "AUD_tmp: SED_NISSFX index", 0, 0x1040));
        } else {
            *tempCharPtr = 0;
        }
