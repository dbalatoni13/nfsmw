...
cFEngRender *cFEngRender::mInstance = nullptr;
float ObjectSortLastZ = -999999.0f;
FEPackage *ObjectSortRenderingPackage = nullptr;
...
void cFEngRender::RenderMultiImage(FEMultiImage *image, FERenderObject *cached, FEPackageRenderInfo *pkg_render_info) {
    ...
    // El objetivo REPITE aqui la conversion entero->float (magic 0x4330...): la
    // primera ya murio al calcular convertu_m/convertv_m y la ranura de pila del
    // round-trip (0xf8) quedo pisada por las conversiones de t2w_m/t2h_m, asi que
    // CSE no la puede reutilizar. Reasignar ftw_m/fth_m alarga su rango de vida y
    // permuta el reparto FP; con locales NUEVAS el reparto sale identico.
    float ftw2_m = static_cast<float>(tw_m);
    float fth2_m = static_cast<float>(th_m);
    uvs[0].x = ss2 * ftw2_m;
    uvs[0].y = sst2 * fth2_m;
    uvs[1].x = ss3 * ftw2_m;
    uvs[1].y = sst2 * fth2_m;
    ...
    rotate_uvs(uvs, bDegToRad(image_data->PivotRot.z), image_data->PivotRot.x * ftw2_m - ftw2_m * 0.5f, image_data->PivotRot.y * fth2_m - fth2_m * 0.5f);
...
void cFEngRender::GenerateRenderContext(unsigned short ctx, FEObject *obj) {
    ...
    RenderContext &Con = RContexts[ctx];
    FEColor color;
    MakeRenderMatrix(reinterpret_cast<FEObjData *>(obj->pData), &Con.matrix, color, obj->RenderContext, 1.0f);
    Con.b = static_cast<uint8>(bClamp(color.b, 0, 255));
    Con.g = static_cast<uint8>(bClamp(color.g, 0, 255));
    Con.r = static_cast<uint8>(bClamp(color.r, 0, 255));
    Con.a = static_cast<uint8>(bClamp(color.a, 0, 255));

    if (Con.group != reinterpret_cast<FEGroup *>(obj)) {
        Con.group = reinterpret_cast<FEGroup *>(obj);
        Con.clipObject = nullptr;
        for (obj = reinterpret_cast<FEGroup *>(obj)->GetFirstChild(); obj; obj = obj->GetNext()) {
        }
    }
}
