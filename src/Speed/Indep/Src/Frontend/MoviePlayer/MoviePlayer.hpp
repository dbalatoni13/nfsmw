...
struct Shape {
    // Functions
    // GetTexture__CQ29RealShape5Shape en el ELF; la usa Movie_GC.cpp.
...
class MoviePlayer {
  public:
    class Settings { // Decl: speed/indep/src/frontend/movieplayer/MoviePlayer.hpp:95
      public:
        // El volcado DWARF del original expande `inline Settings::Settings()` con
        // rango dentro de FEngMovieStarter::Callback: la inicializacion de los
        // campos que aqui estaba en el cuerpo es la del constructor.
        Settings() {
            bufferSize = 0x40000;
            activeController = 0;
            preload = false;
