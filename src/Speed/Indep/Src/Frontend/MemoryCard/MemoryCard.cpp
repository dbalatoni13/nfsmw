...
MemoryCardMessage::MemoryCardMessage(const int* msg, unsigned int nOptions, const int** options) {
...
void MemoryCard::Init() {
    ...
    m_pImp = &sMemcardImp;
    bStrCpy(reinterpret_cast< unsigned short* >(m_GameTitle), "Need for Speed\x99 Most Wanted");
...
void MemoryCard::SetAutoSaveEnabled(bool bEnabled) {
    ...
    if (!Joylog::IsReplaying()) {
        m_pIMemcard->SetAutosave(
            bDisabling ? RealmcIface::AUTOSAVE_DISABLE : RealmcIface::AUTOSAVE_ENABLE,
...
void MemoryCard::ShowMessages(bool bShow) {
    ...
    m_pIMemcard->SetMessage(
        bShow ? RealmcIface::MESSAGE_SHOW : RealmcIface::MESSAGE_HIDE, 0xffffffff);
}
