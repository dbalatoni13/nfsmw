...
void MemoryCard::SetExtraParam(SaveType t, const char* filename, void* buf,
                               unsigned int size) {
...
void MemoryCard::ProcessTask() {
    if (GetScreen() != nullptr) {
        if (m_ReqOp == MO_Delete) {
            Delete(m_ReqFilename);
        } else if (m_ReqOp == MO_Load) {
            Load(m_ReqFilename);
        } else if (m_ReqOp == MO_List) {
...
bool MemoryCard::IsCardBusy() {
    ...
    if (s_pThis->m_pIMemcard->IsResettable()
        ...
        && (s_pThis->m_bInAutoSave == false
            || s_pThis->m_bWaitingForResponse != false)) {
        return false;
    }
    return true;
}
...
void MemoryCard::Init() {
    ...
    m_pImp = &sMemcardImp;
    bStrCpy(reinterpret_cast< unsigned short* >(m_GameTitle),
            "Need for Speed\x99 Most Wanted");
    GameInfo* pGameInfo = new GameInfo(
        reinterpret_cast< unsigned short* >(m_GameTitle), 0, false, false);
    m_pGameInfo = pGameInfo;
    m_pIMemcard = RealmcIface::MemcardInterface::CreateInstance(
        &iSystem, &gMemcardCallbacks, pGameInfo);
...
const char* MemoryCard::GetLocaleString(int strID) {
    return LOCALE_getstrA(s_pThis->m_pLocaleFileHandler, strID);
}

void MemoryCard::SetMessageMode(unsigned int msg, bool flag) {
    if (s_pThis != nullptr) {
        s_pThis->m_pIMemcard->SetMessage(
            flag ? RealmcIface::MESSAGE_SHOW : RealmcIface::MESSAGE_HIDE, msg);
    }
}
...
void MemoryCard::Tick(int TickCount) {
    ...
    if (m_bAutoSaveRequested && m_bHUDLoaded && GManager::Exists()
        && !GManager::Get().GetHasPendingSMS()) {
        m_bAutoSaveRequested = false;
        m_bHUDLoaded = false;
        StartAutoSave(false);
    }
    ...
    if (!cFEng::Get()->IsPackagePushed("ScreenPrintf")
        ...
        if (!FEManager::Get()->IsAllowingControllerError()
            && TheGameFlowManager.GetState() != 6) {
            return;
        }
        if (cFEng::Get()->IsPackagePushed("IG_Pause.fng")
            || cFEng::Get()->IsPackagePushed("AutoSaveIcon.fng")) {
...
void MemoryCard::MessageDone(MessageChoices nInput) {
...
void MemoryCard::BootupCheck(const char* entry) {
    ...
    m_BootupParams.mSaveReqs =
        reinterpret_cast< RealmcIface::SaveReq** >(m_pImp->GetSaveReqArray());
    m_BootupParams.mValidCardIds = 1;
    m_BootupParams.mNumSaveTypes = 1;
    InitCommand(MO_BootUp);
    if (!Joylog::IsReplaying()) {
        m_pIMemcard->BootupCheck(&m_BootupParams, 0,
...
void MemoryCard::SetMonitor(bool bEnabled) {
    ...
    if (!Joylog::IsReplaying()) {
        m_pIMemcard->SetMonitor(
            bEnabled ? RealmcIface::MONITOR_ON : RealmcIface::MONITOR_OFF);
    }
...
void MemoryCard::SetAutoSaveEnabled(bool bEnabled) {
    ...
    if (m_pFEScreen == nullptr || gMemcardSetup.GetMethod() != 0xa) {
        ShowMessages(false);
    } else {
    ...
    if (bDisabling) {
        ...
        gMemcardSetupPreviousOp = gMemcardSetup.mOp & 0xf0;
        gMemcardSetup.ClearMethod();
        gMemcardSetup.SetMethod(0xa);
    }

    InitCommand(MO_AutoSave);
    if (!Joylog::IsReplaying()) {
        m_pIMemcard->SetAutosave(
            bDisabling ? RealmcIface::AUTOSAVE_DISABLE
                       : RealmcIface::AUTOSAVE_ENABLE,
            0, nullptr, entryname, RealmcIface::CARD_UNKNOWN);
    }
    if (bDisabling && Joylog::IsReplaying()) {
        ReplayJoyOp();
    }
}
...
void MemoryCard::ShowMessages(bool bShow) {
    m_bManualSave = bShow;
    m_pIMemcard->SetMessage(
        bShow ? RealmcIface::MESSAGE_SHOW : RealmcIface::MESSAGE_HIDE,
        0xffffffff);
}
...
void MemoryCard::Save(const char* entryName) {
    ...
    if (!Joylog::IsReplaying()) {
        m_pIMemcard->Save(
            m_Filename, GetHeader(), GetData(),
            reinterpret_cast< const RealmcIface::SaveInfo* >(
                m_pImp->GetSaveInfo()),
...
void MemoryCard::Delete(const char* filename) {
    ...
    if (!Joylog::IsReplaying()) {
        m_pIMemcard->Delete(m_Filename, MemoryCardImp::gContentName);
    }
}
...
void MemoryCard::ShowAutoSaveIcon() {
    ...
    if (!cFEng::Get()->IsPackagePushed("AutoSaveIcon.fng")) {
        cFEng::Get()->PushNoControlPackage("AutoSaveIcon.fng",
                                           static_cast< FE_PACKAGE_PRIORITY >(0x68));
    }
    ...
    if (GRaceStatus::Exists()
        ...
        if (cFEng::Get()->IsPackagePushed("SMS_HUD.fng")
            || GManager::Get().GetHasPendingSMS()) {
...
bool MemoryCard::IsAutoSaveIconVisible() {
    ...
    if (FEngIsScriptSet("AutoSaveIcon.fng", obj, script1)) {
        return true;
    }
    unsigned int script2 = FEHashUpper("Idle");
    if (FEngIsScriptSet("AutoSaveIcon.fng", obj, script2)) {
        return true;
    }
    return false;
}
