...
// CON inicializador: el objetivo lo tiene en .data:0x8041BC68 con valor 0, no en
// .bss (regla de GCC 2.9: `X = 0;` va a .data, `X;` a COMMON/.bss).  Ahi va
// pegado detras de RadarDetector::mStaticRange, que es el orden de includes de
// zFe2.cpp (FeRadarDetector.cpp #12, FeMinimapStreamer.cpp #16).
ChoppedMiniMapManager *gChoppedMiniMapManager = nullptr;
...
void ChoppedMiniMapManager::UncompressMaps(short *chop_nums, int num_chops) {
    for (int n = 0; n < NumSections; n++) {
        ...
        if (map->Chunks && !bIsInTable(chop_nums, num_chops, map->ChopNum)) {
