...
// .data:0x8041BC60 (4 B, scope:global, float) = 0.1f, leido del ELF original.
// Es el PRIMER simbolo del rango .data de zFe2 y va pegado delante de
// RadarDetector::mStaticRange (.data:0x8041BC64), asi que la definicion del
// original estaba exactamente aqui.  No es `const`: GCC no puede plegar la carga
// aunque el inicializador sea visible en los usos de mas abajo.
float TWK_RadarDetectorMinThreshold = 0.1f;

// .data:0x8041BC64 con valor 0 en el objetivo, no COMMON/.bss: el original le
// dio inicializador explicito.
float RadarDetector::mStaticRange = 0.0f;
