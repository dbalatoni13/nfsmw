...
void LeaderBoard::Update(IPlayer *player) {
    if (player->GetSettings()->LeaderboardOn) {
        if (!FEngIsScriptSet(mDataLeaderboardGroup, FEHASH_INIT)) {
            FEngSetScript(mDataLeaderboardGroup, FEHASH_INIT, true);
        }
        ...
        if (mNumFramesBeforeTogglingPlayerTimes <= 0) {
            ...
            for (int i = 0; i < mNumRacers && i < 4; i++) {
                if (mNumFramesBeforeTogglingPlayerTimes == 0) {
                    FEngSetScript(mDataRacerText[i], FEHASH_LEAVE, true);
                } else {
                    if (FEngIsScriptSet(mDataRacerText[i], FEHASH_LEAVE) && !FEngIsScriptRunning(mDataRacerText[i], FEHASH_LEAVE) &&
                ...
                if (mShowingRacerTimes) {
                    if (mTopRacers[i].mIsBusted) {
                        ...
                        if (mTopRacers[i].mPercentComplete >= mTopRacers[mPlayerIndex].mPercentComplete) {
                            ...
                            FEPrintf(mDataRacerText[i], "+%$0.0f %s", totalRaceLenMetres * pctDiff, GetTranslatedString(unit));
                        } else {
        ...
        for (int i = 0; i <= 3; i++) {
            if (i == mPlayerIndex) {
                if (!FEngIsScriptSet(mDataRacerTextBackings[i], FEHASH_HIGHLIGHT)) {
                    FEngSetScript(mDataRacerTextBackings[i], FEHASH_HIGHLIGHT, true);
                }
            } else {
                if (!FEngIsScriptSet(mDataRacerTextBackings[i], FEHASH_INIT)) {
                    FEngSetScript(mDataRacerTextBackings[i], FEHASH_INIT, true);
                }
            }
        }
    } else {
        if (!FEngIsScriptSet(mDataLeaderboardGroup, FEHASH_HIDE)) {
            FEngSetScript(mDataLeaderboardGroup, FEHASH_HIDE, true);
        }
    }
...
bool LeaderBoard::ShowSplitTime(IPlayer *player) {
    ...
    int hash;
    if (mPlayerIndex == 0) {
        bSNPrintf(messageString, 32, "%s\n+%s", GetTranslatedString(LANGUAGE_HUD_SPLIT_TIME), timeToPrint);
        hash = FEHASH_ZOOMINGREEN;
    } else {
        bSNPrintf(messageString, 32, "%s\n-%s", GetTranslatedString(LANGUAGE_HUD_SPLIT_TIME), timeToPrint);
        hash = FEHASH_ZOOMINRED;
    }
