...
class HeliItem : public CopItem {
  public:
    HeliItem(FEImage *view, FEObject *icon, bVector2 &pos, bVector2 &world_pos, float rot) : CopItem(icon, pos, world_pos, rot, WMIT_COP_HELI) {
        pViewCone = view;
...
class ItemTypeToggle : public FEButtonWidget {
  public:
    ItemTypeToggle(unsigned int name_hash, eWorldMapItemType type, bool vis) : FEButtonWidget(true) {
        ItemType = type;
        NameHash = name_hash;
        pIcon = nullptr;
        bVisibility = vis;
