#include "Speed/Indep/Src/Frontend/MenuScreens/InGame/TuningSlider.cpp"
...
CustomTuningScreen::CustomTuningScreen(ScreenConstructorData *sd)
    : UIWidgetMenu(sd), HelpTextScroller(nullptr), HelpScrollBar(nullptr), TuningRecord(nullptr), CurrentTuningType(0), HelpVisible(0),
      ExitWithStart(false) {
    HelpScrollBar = new ("", 0) FEScrollBar(GetPackageName(), "Scrollbar", true, true, false);
...
inline CustomTuningScreen::~CustomTuningScreen() {
...
void CustomTuningScreen::NotificationMessage(u32 msg, FEObject *pobj, u32 param1, u32 param2) {
    ...
    if ((msg != 0x9120409E && msg != 0xB5971BF1 && msg != 0x72619778 && msg != 0x911C0A4B) || HelpVisible == 0) {
    ...
    case 0x911AB364:
        if (HelpVisible != 0) {
            ...
            DialogInterface::ShowTwoButtons(GetPackageName(), "InGameDialog.fng", dialog_alert, LANGUAGE_COMMON_YES, LANGUAGE_COMMON_NO, 0x775DBA97,
                                            0x34DC1BCF, 0x34DC1BCF, first_dialog_button2, GetLocalizedString(0xE9CB802F));
        } else {
            cFEng::Get()->QueuePackageMessage(0x587C018B, GetPackageName(), nullptr);
        }
        break;
...
void CustomTuningScreen::ScrollTypes(eScrollDir dir) {
    ...
    if (type != CurrentTuningType) {
        CurrentTuningType = type;
        SetSlidersForType();
    }
}
...
uint32 CustomTuningScreen::AddTuningSlider(FEPlayerCarDB *stable, FECarRecord *record, Physics::Tunings::Path path, bool turbo) {
    ...
    FEngSNPrintf(sztemp, 0x40, "%sGROUP%d", pSliderName, iIndexToAdd);
    ...
    float img_left = FEngGetTopLeftX(reinterpret_cast<FEObject *>(slider->GetRightImage()));
    float img_right = FEngGetSizeX(reinterpret_cast<FEObject *>(slider->GetRightImage()));
    slider->SetWidth(bAbs(img_left + img_right - img_left));
...
void CustomTuningScreen::Setup() {
    ...
    if (Physics::Info::InductionType(vehicle) == Physics::Info::INDUCTION_TURBO_CHARGER) {
...
void CustomTuningScreen::SetSlidersForType() {
    ...
    for (TuningSlider *slider = static_cast<TuningSlider *>(Options.GetHead()); slider != Options.EndOfList();
        ...
        slider->SetSliderValues(lower_limit, upper_limit, (upper_limit - lower_limit) * 0.1f, tunings->Value[slider->TuningPath]);
        slider->Draw();
    }
}
...
void CustomTuningScreen::ShowHelpBlurb() {
    TuningSlider *slider = static_cast<TuningSlider *>(pCurrentOption);
    const unsigned int SHOWHELP = 0x89D332A9;
    cFEng::Get()->QueuePackageMessage(SHOWHELP, GetPackageName(), nullptr);
    static_cast<TuningSlider *>(pCurrentOption)->UnsetFocus();
...
void CustomTuningScreen::HideHelpBlurb() {
    const unsigned int HIDEHELP = 0x950AD1C2;
    cFEng::Get()->QueuePackageMessage(HIDEHELP, GetPackageName(), nullptr);
    static_cast<TuningSlider *>(pCurrentOption)->SetFocus(GetPackageName());
...
bool CustomTuningScreen::SettingsDidNotChange() {
    for (int i = 0; i <= 2; i++) {
        for (int j = 0; j <= 6; j++) {
            float a = TuningRecord->GetTunings(i)->Value[j];
            float b = TempTuningRecord.GetTunings(i)->Value[j];
