    ...
    if (INIS::Get()) {
        if (INIS::Get()->GetType() == CAnimChooser::Arrest) {
            ...
            switch (languageHashToUse) {
                case 0x626ac042:
                    FEPrintf(GetPackageName(), static_cast<int>(0xf1ed545a), GetLocalizedString(0x626ac042), pd.mNumCopsDamaged, pd.mNumCopsDestroyed,
                             pd.mNumRoadblocksDodged, pd.mCostToStateAchieved, finishTimerString);
                    break;
                case 0x626ac043:
                    ...
                    break;
                case 0x626ac044:
                    FEPrintf(GetPackageName(), static_cast<int>(0xf1ed545a), GetLocalizedString(0x626ac044), finishTimerString, pd.mNumCopsDamaged,
                             pd.mNumCopsDestroyed, GInfractionManager::Get().GetNumInfractions());
                    break;
                case 0x626ac045:
                    FEPrintf(GetPackageName(), static_cast<int>(0xf1ed545a), GetLocalizedString(0x626ac045), finishTimerString,
                             pd.mNumSpikeStripsDodged, pd.mNumRoadblocksDodged, GInfractionManager::Get().GetNumInfractions());
                    break;
                case 0x626ac046:
                    FEPrintf(GetPackageName(), static_cast<int>(0xf1ed545a), GetLocalizedString(0x626ac046), finishTimerString,
                             pd.mCostToStateAchieved, GInfractionManager::Get().GetNumInfractions());
                    break;
            }
