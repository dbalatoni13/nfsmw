...
char *Chyron::mTitle = "Uninitialized";
char *Chyron::mArtist = "Uninitialized";
char *Chyron::mAlbum = "Uninitialized";

Chyron::Chyron(ScreenConstructorData *sd) : MenuScreen(sd), mDelayTimer(0) {}

inline void *Chyron::operator new(size_t size) {
    return ChyronScreenPtr;
}

void InitChyron() {
    void *mem = bMalloc(sizeof(Chyron), "Chyron", 0, 0);
    ChyronScreenPtr = static_cast<MenuScreen *>(mem);
}
...
void Chyron::Start() {
    ...
    maxString = bMax(maxString, font->GetTextWidth(reinterpret_cast<const short *>(widestring), artist->Flags));

    bStrCpy(widestring, mAlbum);
    font = FindFont(album->Handle);
    maxString = bMax(maxString, font->GetTextWidth(reinterpret_cast<const short *>(widestring), album->Flags));
