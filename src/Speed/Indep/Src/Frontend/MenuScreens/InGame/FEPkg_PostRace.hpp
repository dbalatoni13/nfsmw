...
class TimerStat : public RaceStat {
  public:
    TimerStat(FEString *title, FEString *data, float seconds, uint32 title_hash) : RaceStat(title, data), Seconds(seconds) {
        TitleHash = title_hash;
    }
...
class SpeedStat : public ResultStat {
  public:
    ...
    void Draw() override {
        ...
        float scaled = speed * (FEDatabase->GetGameplaySettings()->SpeedoUnits == 1 ? 3.6f : 2.23699f);

        FEPrintf(Position, "%d", PosNum);
        FEPrintf(GetDataObject(), "%0.0f", scaled);
    };
...
class PursuitData {
  public:
    ...
    // (la tiene el arbol y no la rama; adenda 11: se repone)
    int GetPursuitIsActive() const {
        return mPursuitIsActive;
    }
