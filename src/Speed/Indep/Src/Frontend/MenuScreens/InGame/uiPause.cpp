...
message_E1FDE1D1:
    if (PrevButtonMessage != 0x911AB364) {
        switch (mSelectionHash) {
            case 0xFBDF2EE3:
                if (GRaceStatus::Exists() && GRaceStatus::Get().GetRaceParameters() && GRaceStatus::Get().GetRaceParameters()->GetIsDDayRace()) {
            ...
            case 0xE5C9C609: {
                ...
                new EQuitToFE(GRaceStatus::Get().GetRaceContext() == GRace::kRaceContext_Career ? static_cast<eGarageType>(2)
