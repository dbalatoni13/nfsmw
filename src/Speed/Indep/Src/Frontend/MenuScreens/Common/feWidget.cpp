...
FEButtonWidget::FEButtonWidget(bool enabled) : FEWidget(nullptr, enabled, false), pTitle(nullptr), vMaxTitleSize(0.0f, 0.0f) {}
...
void FEButtonWidget::Position() {
    ...
    if ((alignment & 1) != 0) {
        ...
        FEngSetCenter(reinterpret_cast<FEObject *>(pTitle), GetTopLeftX() + vMaxTitleSize.x, GetTopLeftY() + vMaxTitleSize.y * 0.5f);
    } else {
...
void FEButtonWidget::SetFocus(const char *parent_pkg) {
    ...
    FEngSetScript(reinterpret_cast<FEObject *>(GetTitleObject()), FEHASH_HIGHLIGHT, true);
    if (GetBacking()) {
...
void FEButtonWidget::UnsetFocus() {
    FEngSetScript(reinterpret_cast<FEObject *>(GetTitleObject()), FEHASH_UNHIGHLIGHT, true);
    if (GetBacking()) {
...
void FEStatWidget::Position() {
    ...
    if (pData) {
        ...
        if ((format & 1) != 0) {
            FEngSetCenter(data, GetDataPosX() + GetMaxDataWidth() * 0.5f, title_y);
        } else if ((format & 2) != 0) {
            FEngSetCenter(data, FEngGetCenterX(data), title_y);

            FEVector3 pos = reinterpret_cast<FEObject *>(pData)->GetObjData()->Pos;
            pos.x = GetDataPosX() + GetMaxDataWidth();
            reinterpret_cast<FEObject *>(GetDataObject())->SetPosition(pos, false);
        } else {
...
FEToggleWidget::FEToggleWidget(bool enabled) : FEStatWidget(enabled) {
    pLeftImage = nullptr;
    pRightImage = nullptr;
    EnableScript = FEHASH_UNHIGHLIGHT;
    DisableScript = FEHASH_DISABLE;
}
...
FEScrollBar::FEScrollBar(const char *parent_pkg, const char *name, bool vert, bool resize, bool arrows_only)
    : bVertical(vert),             //
      ...
      bVisible(false),             //
      vGrabbedPos(0.0f, 0.0f),     //
      vCurPos(0.0f, 0.0f),         //
      vGrabOffset(0.0f, 0.0f),     //
      vBackingPos(0.0f, 0.0f),     //
      vBackingSize(0.0f, 0.0f),    //
      vHandleMinSize(0.0f, 0.0f),  //
      pBacking(nullptr),           //
      pHandle(nullptr),            //
      pFirstArrow(nullptr),        //
    ...
    if (name) {
        ...
        if (bResizeHandle) {
            if (bVertical) {
                vHandleMinSize.y = vBackingSize.x;
                vHandleMinSize.x = vBackingSize.x;
            } else {
                vHandleMinSize.y = vBackingSize.y;
...
void FEScrollBar::SetArrow1Dim(bool dim) {
    FEngSetScript(pFirstArrow, dim ? 0x9E99 : FEHASH_NORMAL, true);
}

void FEScrollBar::SetArrow2Dim(bool dim) {
    FEngSetScript(pSecondArrow, dim ? 0x9E99 : FEHASH_NORMAL, true);
}
...
void CTextScroller::SetText(int16 *pText) {
    if (m_pFont != nullptr && pText != nullptr) {
        int16 *pTextPtr = pText;
        int16 *pNextCR = nullptr;
        int NumLines = 0;
        int NumChars = 0;
        ...
        pNextCR = FindCR(pTextPtr);

        if (!pNextCR) {
            pNextCR = FindEND(pTextPtr);
        }

        while (pNextCR != nullptr && *pTextPtr != 0) {
            if (pNextCR == pTextPtr) {
                NumLines++;
                NumChars++;
            } else {
                int ThisSectionNumLines;
                int ThisSectionNumCharacters;

                WordWrapCountLinesAndChars(pTextPtr, pNextCR, ThisSectionNumLines, ThisSectionNumCharacters);

                NumLines += ThisSectionNumLines;
                NumChars += ThisSectionNumCharacters;
            }
        ...
        if (NumLines > 0) {
            if (!m_pRawDataBlock) {
                m_DataBlockSize = NumChars * 2 + NumLines * 4;
                m_pRawDataBlock = new char[m_DataBlockSize];
            }

            m_pLines = reinterpret_cast<int16 **>(m_pRawDataBlock);
            m_DataBlockCurPos = NumLines * 4;
            ...
            while (pNextCR != nullptr && *pTextPtr != 0) {
                if (pNextCR == pTextPtr) {
                    ...
                    WordWrapAddLines(pTextPtr, pNextCR, false, nullptr);
                }
...
void CTextScroller::Display(int32 topLine) {
    ...
    for (int i = 0; i < m_ViewVisibleLines; i++) {
        ...
        if (lineCount < m_NumAddedLines) {
            ...
            reinterpret_cast<FEObject *>(str)->Flags |= FF_Dirty;
        } else {
            FEString *str = FEngFindString(m_pOwner->GetPackageName(), FEHashUpper(textBoxName));
...
void CTextScroller::AddLine(int16 *pText, int numChars) {
    if (m_DataBlockCurPos > m_DataBlockSize - (numChars + 1) * 2) {
        return;
    }
    m_pLines[m_NumAddedLines] = reinterpret_cast<int16 *>(m_pRawDataBlock + m_DataBlockCurPos);
    memcpy(m_pLines[m_NumAddedLines], pText, numChars * 2);
...
int CTextScroller::WordWrapAddLines(int16 *pTextStart, int16 *pTextEnd, bool bCountOnly, int *pNumCharsOut) {
    ...
    while (pTextEnd > pTextStart) {
        int StringLength = pTextEnd - pTextStart;
        int StringSize = 0;
        ...
        while (TextWidth < static_cast<float>(m_ViewWidth - 16) && pChar < pTextEnd && !IsNewlineChar(*pChar) &&
               !bStringSizeOverflow) {
