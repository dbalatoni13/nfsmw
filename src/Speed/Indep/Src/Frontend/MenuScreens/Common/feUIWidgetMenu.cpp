...
UIWidgetMenu::UIWidgetMenu(ScreenConstructorData *sd)
    : MenuScreen(sd),          //
      ...
      pSliderName("SLIDER_"),                                        //
      vWidgetStartPos(0.0f, 0.0f),                                   //
      vLastWidgetPos(0.0f, 0.0f),                                    //
      vWidgetSize(0.0f, 0.0f),                                       //
      vMaxTitleSize(175.0f, 24.0f),                                  //
      vMaxDataSize(175.0f, 24.0f),                                   //
      vDataPos(0.0f, 0.0f),                                          //
      vWidgetSpacing(4.0f, 0.0f) {
    iIndexToAdd = 1;
...
eMenuSoundTriggers UIWidgetMenu::NotifySoundMessage(u32 msg, eMenuSoundTriggers maybe) {
    if ((msg == 0x48122792 || msg == 0x4AC5E165) && pCurrentOption && !pCurrentOption->MovedLastUpdate()) {
        ...
        if (new_option && new_option == Options.GetTail() && pDone) {
            ...
            if (pCurrentOption != Options.GetTail()) {
                do {
                    ...
                    iLastSelectedIndex = bMin(static_cast<int>(iLastSelectedIndex + 1), static_cast<int>(iIndexToAdd - 1));
...
void UIWidgetMenu::IncrementStartPos() {
    float y = vLastWidgetPos.y + (vWidgetSize.y + vWidgetSpacing.y);
    vLastWidgetPos.y = y;
    vDataPos.y = y;
}
...
void UIWidgetMenu::SyncViewToSelection() {
    ...
    if (iIndexToAdd - 1 > iMaxWidgetsOnScreen && GetWidgetIndex(pCurrentOption) <= iIndexToAdd - iMaxWidgetsOnScreen) {
        ...
        unsigned int index = bMax(0, static_cast<int>(iIndexToAdd - iMaxWidgetsOnScreen) - 1);
        pViewTop = static_cast<FEWidget *>(Options.GetNode(index));
    }
    Reposition();
    bViewNeedsSync = false;
}
