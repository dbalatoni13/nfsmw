...
void Scrollerina::ScrollNext() {
    if (bWrapped) {
        ...
        g_pEAXSound->PlayUISoundFX(GetSelectedDatum() == GetLastDatum() ? static_cast<eMenuSoundTriggers>(7)
                                              : static_cast<eMenuSoundTriggers>(0));
        Scroll(eSD_NEXT);
    }
}
...
void Scrollerina::ScrollPrev() {
    if (bWrapped) {
        ...
        g_pEAXSound->PlayUISoundFX(GetSelectedDatum() == GetFirstDatum() ? static_cast<eMenuSoundTriggers>(7)
                                              : static_cast<eMenuSoundTriggers>(0));
        Scroll(eSD_PREV);
    }
}
...
bool Scrollerina::ScrollSelection(eScrollDir dir) {
    ...
    if (slot != SelectedSlot) {
        SelectedSlot->SetScript(FEHASH_UNHIGHLIGHT);
        SelectedSlot = slot;
        slot->SetScript(FEHASH_HIGHLIGHT);
        ret = true;
    }
    return ret;
}
...
void Scrollerina::Print() {
    ...
    for (ScrollerSlot *slot = Slots.GetHead(); slot != Slots.EndOfList(); slot = slot->GetNext()) {
        ...
        if (datum != nullptr && datum != Data.EndOfList()) {
            ...
            for (ScrollerSlotNode *slot_string = slot->FEStrings.GetHead();
                 slot_string != slot->FEStrings.EndOfList(); slot_string = slot_string->GetNext()) {
                ...
                if (data_string == datum->Strings.EndOfList() &&
                    slot_string->GetNext() != slot->FEStrings.EndOfList()) {
                    break;
                }
            }
...
void Scrollerina::Enable(ScrollerDatum *datum) {
    if (!datum || Slots.IsEmpty() || Data.IsEmpty()) {
        return;
    }
    datum->Enable();
