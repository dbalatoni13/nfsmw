...
extern bool CarGuysCamera;            // .data 0x8041EED4
extern int32 CarTypeInfoArrayUpdated; // World/CarInfo.cpp
void SetSelectCarLighting(int view_id, float intensity, int which);
...
static Attrib::Key FindScreenInfo(const char *pkg_name, int category) {
    ...
    if (len > 3) {
        ...
        if (FEDatabase->GetGameMode() & 0x20) {
            ...
            {
                Attrib::Gen::frontend inst(key, 0, nullptr);
        ...
        } else if (FEDatabase->GetGameMode() & 0x8000) {
            bStrCat(prefix, "carlot_", name);
        } else if (FEDatabase->GetGameMode() & 1) {
            bStrCat(prefix, "career_", name);
        } else if (FEDatabase->GetGameMode() & 4 && FEDatabase->GetGameMode() & 0x400) {
            bStrCat(prefix, "quickrace_", name);
        } else if (FEDatabase->GetGameMode() & 4) {
            bStrCat(prefix, "quickracemain_", name);
        } else if (FEDatabase->GetGameMode() & 8 || FEDatabase->GetGameMode() & 0x40) {
            bStrCat(prefix, "online_", name);
        } else if (FEDatabase->GetGameMode() & 0x10) {
            bStrCat(prefix, "options_", name);
        } else if (FEDatabase->GetGameMode() & 0x100) {
...
void GarageMainScreen::UpdateCurrentCameraView(bool bForce) {
    if (CameraPushRequested || bForce) {
        ...
        Attrib::Gen::frontend entry(entryKey, 0, nullptr);
        ...
        Attrib::Gen::frontend final_cam(finalKey, 0, nullptr);
...
void GarageMainScreen::UpdateRenderingCarParameters(FrontEndRenderingCar *fe_car) {
    ...
    if (reinterpret_cast<int>(fe_car) == -8 || loaded_ride_info->Type == static_cast<CarType>(-1) || HideEntireScreen) {
        fe_car->Visible = 0;
        return;
    }
...
// Los siete metodos de CarViewer y haveLoadedOnce viven en FECarViewer.cpp.
