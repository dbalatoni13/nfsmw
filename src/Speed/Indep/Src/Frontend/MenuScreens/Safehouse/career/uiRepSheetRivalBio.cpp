...
void uiRepSheetRivalBio::NotificationMessage(u32 msg, FEObject* obj, u32 param1,
                                              u32 param2) {
    switch (msg) {
    case 0xc519bfbf:
        if (FEDatabase->IsPostRivalMode()) {
...
void uiRepSheetRivalBio::RefreshHeader() {
    ...
    unsigned int hash = FEngHashString("BLACKLIST_RIVAL_%02d_NAME", iCurrentViewBin);
    FEngSetLanguageHash(pkgName, 0x7ac3d0c9, hash);
    pkgName = GetPackageName();
    hash = FEngHashString("BLACKLIST_RIVAL_%02d_AKA", iCurrentViewBin);
    FEngSetLanguageHash(pkgName, 0xb1f2748d, hash);
    pkgName = GetPackageName();
    hash = FEngHashString("BLACKLIST_RIVAL_%02d_BIO", iCurrentViewBin);
    FEngSetLanguageHash(pkgName, 0x27e1d6d8, hash);
    pkgName = GetPackageName();
    hash = FEngHashString("BLACKLIST_RIVAL_%02d_CAR", iCurrentViewBin);
    FEngSetLanguageHash(pkgName, 0xcb5bf41a, hash);
    pkgName = GetPackageName();
    hash = FEngHashString("BLACKLIST_RIVAL_%02d_STRENGTH", iCurrentViewBin);
