...
void uiCareerManager::Setup() {
    ...
    if (FEDatabase->bProfileLoaded) {
        ...
        FEPrintf(GetPackageName(), 0xEB406FEC, "%s", FEDatabase->GetUserProfile(0)->GetProfileName());
    }
...
inline void CResumeCareer::React(const char *pkg_name, uint32 data, FEObject *obj, uint32 param1, uint32 param2) {
    if (data == 0x0C407210) {
...
inline void CStartNewCareer::React(const char *pkg_name, uint32 data, FEObject *obj, uint32 param1, uint32 param2) {
    if (data == 0x0C407210) {
        cFrontendDatabase *db = FEDatabase;
        signed char port = FEngMapJoyParamToJoyport(param1);
        db->SetPlayersJoystickPort(0, port);
...
inline void CLoadCareer::React(const char *pkg_name, uint32 data, FEObject *obj, uint32 param1, uint32 param2) {
    if (data == 0x0C407210) {
        cFrontendDatabase *db = FEDatabase;
        signed char port = FEngMapJoyParamToJoyport(param1);
        db->SetPlayersJoystickPort(0, port);
