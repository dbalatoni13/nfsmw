...
void uiCareerManager::NotificationMessage(u32 msg, FEObject* pobj, u32 param1,
                                          u32 param2) {
...
void uiCareerManager::Setup() {
    ...
    if (FEDatabase->GetCareerSettings()->HasCareerStarted()) {
        ...
        CStartNewCareer* startNew = new ("CStartNewCareer", 0) CStartNewCareer(0xE7353BE7, 0x17E18F87, 0);
        startNew->SetReactImmediately(true);
        AddOption(startNew);
    } else {
        CStartNewCareer* startNew = new ("CStartNewCareer", 0) CStartNewCareer(0xE7353BE7, 0x6005281E, 0);
        startNew->SetReactImmediately(true);
        AddOption(startNew);
    }

    CLoadCareer* loadCareer = new ("CLoadCareer", 0) CLoadCareer(0x2287E063, 0x18ECFF, 0);
