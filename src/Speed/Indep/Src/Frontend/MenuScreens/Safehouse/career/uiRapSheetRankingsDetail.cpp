...
void RapSheetRankingsArraySlot::Update(ArrayDatum *datum, bool isSelected) {
    ...
    if (datum != nullptr) {
        ...
        if (d->getItemNum() != 0x10) {
            FEPrintf(pItemNum, "%d", d->getItemNum());
        } else {
            FEngSetLanguageHash(pItemNum, 0xFC1BF40);
        }
        if (d->getCarName() != 0) {
            FEngSetLanguageHash(pCarName, d->getCarName());
        } else {
            FEPrintf(pCarName, "");
        }
        if (d->getPlayerName() != 1) {
            FEngSetLanguageHash(pPlayerName, d->getPlayerName());
        } else {
...
uiRapSheetRankingsDetail::uiRapSheetRankingsDetail(ScreenConstructorData *sd)
    ...
    for (int i = 0; i < GetWidth() * GetHeight(); i++) {
        ...
        if (rank_type == PD_COST_TO_STATE) {
            AddSlot(new (__FILE__, __LINE__) RapSheetRankingsTimerArraySlot(pItemNum, pPlayerName, pCarName, pValue));
        } else {
            AddSlot(new (__FILE__, __LINE__) RapSheetRankingsArraySlot(pItemNum, pPlayerName, pCarName, pValue));
        }
    }
    Setup();
}
        ...
        if (last == 15) {
            ...
            for (int i = 0; i < num_rankings_to_show; i++) {
                if (i == player_rank_index) {
                    ...
                    AddDatum(new (__FILE__, __LINE__) RapSheetRankingsDatum(i + 1, name_hash, car_hash, rankingsData.RapSheetRanks(rank_index)));
                }
            }
