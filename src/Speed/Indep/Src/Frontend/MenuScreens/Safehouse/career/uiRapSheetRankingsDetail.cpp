...
void uiRapSheetRankingsDetail::NotificationMessage(unsigned long msg, FEObject* pobj, unsigned long param1, unsigned long param2) {
    ...
    if (msg == 0x911C0A4B || msg == 0x35F8620B || msg == 0x72619778) { UpdateHighlight(); }
    else if (msg == 0xC519BFC4) { career_view = !career_view; Setup(); }
    else if (msg == 0xE1FDE1D1) { uiRapSheetRankings::career_view = career_view; cFEng::Get()->QueuePackageSwitch("RapSheetRankings.fng", 0, 0, false); }
}
