...
int selection = 0;
unsigned int iCurrentViewBin = 0;
...
void uiRepSheetMain::UpdateInfo() {
    ...
    const char *rival_name = GetLocalizedString(FEngHashString("BLACKLIST_RIVAL_%02d_AKA", iCurrentViewBin));
    const char *challenge_blurb = GetLocalizedString(FEngHashString("BLACKLIST_RIVAL_%02d_CHALLENGE", iCurrentViewBin));
    ...
    if (bBossBeaten) {
        ...
        if (bBossAvailable) {
            unsigned int msgHash = FEngHashString("RIVAL_CHALLENGE_ACTIVATED");
            cFEng::Get()->QueuePackageMessage(msgHash, GetPackageName(), nullptr);
            FEngSetVisible(GetPackageName(), 0x55f6aa1a);
        } else {
            unsigned int msgHash = FEngHashString("RIVAL_CHALLENGE_DEACTIVATED");
            cFEng::Get()->QueuePackageMessage(msgHash, GetPackageName(), nullptr);
            FEngSetInvisible(GetPackageName(), 0x55f6aa1a);
        }
    }
...
void uiRepSheetMain::ScrollRival(eScrollDir dir) {
    ...
    if (dir == static_cast<eScrollDir>(1)) {
        iCurrentViewBin--;
        if (static_cast<int>(iCurrentViewBin) < 0 ||
            static_cast<int>(iCurrentViewBin) < FEDatabase->GetCareerSettings()->GetCurrentBin()) {
            iCurrentViewBin = 15;
        }
    ...
    if (old_bin != static_cast<int>(iCurrentViewBin)) {
        if (dir == static_cast<eScrollDir>(1)) {
