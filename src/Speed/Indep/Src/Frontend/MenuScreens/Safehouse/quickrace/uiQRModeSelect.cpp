...
void UIQRModeSelect::Setup() {
    if (GetMikeMannBuild()) {
        MSOption *opt;
        opt = new ("MSOption", 0) MSOption(0xe9638d3e, 0x34fa2c1, GRace::kRaceType_Circuit);
        AddOption(opt);
        if (GetMikeMannBuild() == 1) {
            opt = new ("MSOption", 0) MSOption(0x2521e5eb, 0xb94fd70e, GRace::kRaceType_P2P);
            AddOption(opt);
            opt = new ("MSOption", 0) MSOption(0xaaab31e9, 0x6f547e4c, GRace::kRaceType_Drag);
            AddOption(opt);
            opt = new ("MSOption", 0) MSOption(0x1a091045, 0xa15e4505, GRace::kRaceType_Tollbooth);
            AddOption(opt);
        }
    } else {
        MSOption *opt;
        ...
        if (!FEDatabase->IsOnlineMode() && !FEDatabase->IsLANMode()) {
            ...
            if (!isSplitQR) {
                opt = new ("MSOption", 0) MSOption(0x3a015595, 0x4930f5fc, GRace::kRaceType_Knockout);
                AddOption(opt);
            }
            opt = new ("MSOption", 0) MSOption(0x66c9a7b6, 0xee1edc76, GRace::kRaceType_SpeedTrap);
            AddOption(opt);
        }
    }
