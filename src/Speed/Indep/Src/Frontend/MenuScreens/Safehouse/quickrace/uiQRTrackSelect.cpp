...
void UIQRTrackSelect::NotificationMessage(u32 msg, FEObject *pobj, u32 param1, u32 param2) {
    switch (msg) {
        ...
        case 0x911ab364: {
            ...
            FEDatabase->GetQuickRaceSettings(static_cast<GRace::Type>(0xb))->EventHash = 0;
        ...
        case 0x406415e3:
            ...
            cFEng::Get()->QueuePackageMessage(FEHASH_LEAVE_ACCEPT, GetPackageName(), nullptr);
            break;
        case 0x9120409e:
...
void UIQRTrackSelect::TryToAddTrack(GRaceParameters *parms, int unlock_filter, int bin_num) {
    ...
    if (GetMikeMannBuild()) {
        ...
        unsigned char regionFilter = FEDatabase->GetQuickRaceSettings(static_cast<GRace::Type>(0xb))->RegionFilterBits;
        if (parms->GetRegion() == regionFilter || regionFilter == GRace::kRaceRegion_NumRegions) {
            ...
            unsigned int hash = FEDatabase->GetQuickRaceSettings(static_cast<GRace::Type>(0xb))->EventHash;
...
void UIQRTrackSelect::RefreshHeader() {
    ...
    FEPrintf(GetPackageName(), __NUMBER__, "%d", Tracks.GetNodeNumber(pCurrentNode));
    FEPrintf(GetPackageName(), __NUMBER_OF__, "%d", Tracks.CountElements());
    ...
    if (!pCurrentTrack) {
        FEPrintf(GetPackageName(), __NUMBER__, "0");
        FEPrintf(GetPackageName(), __NUMBER_OF__, "0");
        ...
        if (pCurrentTrack->GetRaceType() == GRace::kRaceType_P2P || pCurrentTrack->GetRaceType() == GRace::kRaceType_Circuit ||
            ...
            if (FEDatabase->GetGameplaySettings()->SpeedoUnits == 1) {
                ...
                max_speed = MPS2MPH(info->mHighScores.mBestSpeed * 0.27778f);
            }
