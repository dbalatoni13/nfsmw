...
void Challenge::React(const char* pkg_name, unsigned int data, FEObject* obj, unsigned int param1, unsigned int param2) {
    ...
    } else if (IsMemcardEnabled) {
        MemcardEnter("MainMenu.fng", "ChallengeSeries.fng", 0x10163, nullptr, nullptr, 0, 0);
    }
}
...
void UIMain::NotificationMessage(unsigned long msg, FEObject* obj, unsigned long param1,
...
void UIMain::UpdateProfileData() {
    if (!FEDatabase->bProfileLoaded) {
        ...
        FEPrintf(GetPackageName(), FEHashUpper("GAME_COMPLETED_DATA"), "%d%s", stats.m_nOverall,
                 szPercentUnit);
        FEPrintf(GetPackageName(), FEHashUpper("CAREER_COMPLETED_DATA"), "%d%s", stats.m_nCareer,
                 szPercentUnit);
        FEngSetInvisible(GetPackageName(), FEHashUpper("RAP_SHEET_ITEMS_DATA_1"));
        FEPrintf(GetPackageName(), FEHashUpper("RAP_SHEET_ITEMS_DATA_2"), "%d%s",
                 stats.m_nRapSheetRankings, szPercentUnit);
        FEPrintf(GetPackageName(), FEHashUpper("CHALLENGE_SERIES_DATA_1"), "%d/%d",
                 stats.m_nCompletedChallengeRaces, stats.m_nTotalChallengeRaces);
        FEPrintf(GetPackageName(), FEHashUpper("CHALLENGE_SERIES_DATA_2"), "%d%s",
                 stats.m_nChallenge, szPercentUnit);
        FEPrintf(GetPackageName(), FEObj_PLAYERNAMEGROUP, "%s",
