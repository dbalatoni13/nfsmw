...
class cFEng {
  public:
    ...
    } // Decl: speed/indep/src/frontend/fenginterfaces/FEngInterface.hpp:43
...
// --- Compat temporal: declaraciones que upstream reparte en FEngFrontend.hpp,
// --- FEManager.hpp, FEngInterfaceFEStrings.hpp y Localize.hpp.  Borrables
// --- cuando esas cabeceras entren en un lote posterior.
int FEngMapJoyParamToJoyport(int feng_param);
int GetPortsPlayer(int port);
unsigned int FEngHashString(const char *format, ...);
void FEngSetLanguageHash(const char *pkg_name, unsigned int string_hash, unsigned int language_hash);
char *GetLocalizedString(unsigned int string_label);
