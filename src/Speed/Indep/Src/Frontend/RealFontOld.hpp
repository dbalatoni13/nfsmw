...
const Glyph *BSearch(wchar_t unicode, const Glyph *table, unsigned int count, int stride = 0);
...
class Font {
  public:
    ...
    const RealFontOld::Glyph *GetGlyph(int unicode) const {
        const RealFontOld::Glyph *ch;
        const RealFontOld::Glyph *GlyphTbl =
            reinterpret_cast<const RealFontOld::Glyph *>(reinterpret_cast<const char *>(this) + mGlyphTbl);
        const int GlyphSize = (mFlags & 0x40000) ? 0x10 : 0x0C;
        if (unicode - 0x20 < mNum) {
            ch = reinterpret_cast<const RealFontOld::Glyph *>(
                reinterpret_cast<const char *>(GlyphTbl) + (unicode - 0x20) * GlyphSize);
            if (ch->mUnicode == unicode) {
                return ch;
            }
        }
        return BSearch(unicode, GlyphTbl, mNum, GlyphSize);
    }
    ...
    int GetKern(const RealFontOld::Glyph *glyph, wchar_t wch) const;
    void GetRect();
    // void GetRect();
    // void GetRect();
    // void GetRect();
