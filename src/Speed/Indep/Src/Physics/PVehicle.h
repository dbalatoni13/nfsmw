...
struct VehicleParams : public Sim::Param {
    ...
    const struct FECustomizationRecord *customization; // offset 0x20, size 0x4
    IVehicleCache *VehicleCache;                       // offset 0x24, size 0x4
    const struct Performance *matched;                 // offset 0x28, size 0x4
    ...
    VehicleParams(IVehicleCache *cache, DriverClass driver, unsigned int type, const UMath::Vector3 &dir, const UMath::Vector3 &pos,
                  unsigned int flags, const FECustomizationRecord *upgrades, const struct Performance *match)
        : Sim::Param(UCrc32(0xa6b47fac), this), carClass(driver), carType(type), initialVec(dir), initialPos(pos), customization(upgrades),
          VehicleCache(cache), matched(match), Flags(flags) {}
