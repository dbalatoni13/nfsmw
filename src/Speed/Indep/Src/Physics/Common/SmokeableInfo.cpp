...
int SceneryModel::mSceneryCount = 4;
...
void SceneryModel::WakeUp() {
    ...
    if (trigger != nullptr && !IsSimulating() && !IsRendering()) {
        trigger->Fire();
        trigger->Disable();
    }
}
...
const Attrib::Collection *SmokeableSpawner::FindAttributes(UCrc32 name) {
    return Attrib::FindCollection(Attrib::Gen::smackable::ClassKey(), name.GetValue());
}
...
bool SceneryModel::IsHidden() const {
    bool visible = mInstanceVisible;
    if (!visible) {
        return Sim::Model::IsHidden();
    }
    return false;
}

bool SceneryModel::IsExcluded(unsigned int scenery_exclusion_flag) const {
    unsigned int my_flags = mSpawner->GetExcludeFlags();
