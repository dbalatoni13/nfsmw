...
void RBTractor::SetHitch(bool hitched) {
    if (hitched) {
        ...
        if (mTrailer && !mHitched && mTrailer->QueryInterface(&ibounds_trailer)) {
            ...
            const CollisionGeometry::Collection *tractor_col = CollisionGeometry::Lookup(UCrc32(GetVehicle()->GetVehicleAttributes().MODEL()));
            const CollisionGeometry::Collection *trailer_col = CollisionGeometry::Lookup(UCrc32(mTrailer->GetVehicleAttributes().MODEL()));
