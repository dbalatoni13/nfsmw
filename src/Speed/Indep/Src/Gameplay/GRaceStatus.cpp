...
void GRaceCustom::CreateRaceActivity() {
    ...
    if (mNumOpponents != static_cast<unsigned int>(existingOpponents)) {
...

        {
            ...
            if (onSet < mNumOpponents) {
                do {
                    GCollectionKey opponent(opponentKey[onSet]);

                    attribute.Set(onSet, opponent);
                    onSet++;
                } while (onSet < mNumOpponents);
            }
        }
    }
...
void GRaceCustom::GetCheckpointDirection(unsigned int index, UMath::Vector3 &dir) const {
    ...
    checkDir = UMath::Vector3::kZero;
    {
        ...
        if (raceCheckSpec.GetCollectionKey()) {
            ...
            if (mReversed) {
                ExtractDirection(checkpoint, checkDir, 180.0f);
            } else {
                ExtractDirection(checkpoint, checkDir, 0.0f);
            }
        }
    }
    dir = checkDir;
...
void GRaceStatus::UpdateAdaptiveDifficulty(eAdaptiveGainReason reason, ISimable *who) {
    ...
    if (static_cast<unsigned int>(reason) < 2) {
        ...
        win_margin = 100.0f - max_pct_complete;
        win_margin *= 0.01f;
        win_margin *= GetRaceLength();
...
bool GRaceStatus::ComputeCatchUpSkill(GRacerInfo *racer_info, PidError *pid, float *output, float *skill, bool off_world) {
    ...
    if (!use_race_override) {
        ...
        glue_p *= glue_integral;
        glue_error *= glue_derivative;
    } else {
        {
    ...
    glue_output = bClamp((2.0f / glue_spread) * pid->GetError() + glue_p + glue_error, -1.0f, 1.0f);
