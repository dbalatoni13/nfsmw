static const unsigned int bCrcTable[256] = {
    0,          0x04C11DB7, 0x09823B6E, 0x0D4326D9, 0x130476DC, 0x17C56B6B, 0x1A864DB2, 0x1E475005, 0x2608EDB8, 0x22C9F00F, 0x2F8AD6D6, 0x2B4BCB61,
...
unsigned int bCalculateCrc32(const void *data, int size, unsigned int prev_crc32) {
    ...
    for (; size > 0; size >>= 8) {
        crc = crc << 8 ^ bCrcTable[(crc >> 0x18 ^ size) & 0xff];
    }
    return ~crc;
}
