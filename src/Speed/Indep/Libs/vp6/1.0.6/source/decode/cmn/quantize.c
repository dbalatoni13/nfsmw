...
extern QTABLE VP6_QThreshTable[64];
extern QTABLE VP6_UvQThreshTable[64];
extern unsigned short VP6_DcQuant[64];
extern unsigned short VP6_UvDcQuant[64];
...
void VP6_init_dequantizer(QUANTIZER *qi, unsigned char Vp3VersionNo) {
    ...
    for (i = 1; i < 64; i++) {
        j = qi->quant_index[i];
        qi->dequant_coeffs[0][j] =
            ((volatile QTABLE *)VP6_QThreshTable)[qi->FrameQIndex].value * 4;
    }
    qi->dequant_coeffs[0][0] = VP6_DcQuant[qi->FrameQIndex] * 4;

    for (i = 1; i < 64; i++) {
        j = qi->quant_index[i];
        qi->dequant_coeffs[1][j] =
            ((volatile QTABLE *)VP6_UvQThreshTable)[qi->FrameQIndex].value * 4;
    }
