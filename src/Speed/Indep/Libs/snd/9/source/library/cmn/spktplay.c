...
// Prototipos que ninguna cabecera compartida declara todavia. Se declaran aqui
// para no tocar sndcmn.h / sndo.h (los comparten unidades ya casadas).
...
int SNDPLATFORM_packetplay(int packetinstancehandle, int voice, int timemult, int lowpasscutoff, int highpasscutoff, SNDSAMPLEFORMAT *pssf,
                           SNDSAMPLEATTR *pssa);
void SNDPLATFORM_getvoicerange(int rendermode, int *pminvoicerange, int *pmaxvoicerange);
...
static const unsigned short SNDI_PACKET_CB_FREEFRAMES = 0;
static const unsigned short SNDI_PACKET_CB_RELEASE = 1;
...
int SNDPKTPLAYI_overhead(int maxpackets) {
    return sizeof(SNDPACKETCHAN) + (maxpackets * sizeof(SNDPACKETENTRY));
}

extern "C" int SNDPKTPLAY_overhead(int maxpackets) {
    int ret;

    ret = SNDPKTPLAYI_overhead(maxpackets);

    return ret + SNDPLATFORM_packetoverhead();
}

extern "C" int SNDPKTPLAY_create(void (*preleasefunc)(void *, void *), void (*pframesfunc)(int, int, void *), void *pclientdata, void *pmem,
                                 int memsize) {
    ...
    for (packetinstancehandle = 0; packetinstancehandle < sndgs.sso.set.maxstreams;
         packetinstancehandle++) {
        if (sndpps.ppc[packetinstancehandle] == NULL) {
            i = packetinstancehandle;
            goto allocated;
        }
    }
...
allocated:
    if (SNDPLATFORM_packetplaycreate(i, pmem) < 0) {
        SNDSYS_leavecritical();
    ...
    pplatformmem = pmem;
    pmem = (char *)pmem + SNDPLATFORM_packetoverhead();
    memsize -= SNDPLATFORM_packetoverhead();

    ppchan = (SNDPACKETCHAN *)pmem;

    ppchan->maxpackets = (memsize - sizeof(SNDPACKETCHAN)) / sizeof(SNDPACKETENTRY);
...
extern "C" int SNDPKTPLAY_start(int packetinstancehandle, SNDSAMPLEFORMAT *pssf, SNDSAMPLEATTR *pssa, SNDPLAYOPTS *pspo) {
    ...
    while (!done && (rendermode = SNDI_validrendermode(&rendermodeindex, &ph)) != 0) {
        CHANPUB *pVoice;

        SNDPLATFORM_getvoicerange(rendermode, &minvoicerange, &maxvoicerange);

        voice = SNDVOICEI_alloc(voicesneeded, 101, &shandle, minvoicerange, maxvoicerange);

        if (voice < 0) {
            ppchan->shandle = SNDERR_ALLOCATE;
            continue;
        }
        ...
        pVoice->pFxVolume->fxLevel = pspo->fxlevel0 * 0.007874016f;
        ...
        pVoice->programmedVol = pspo->vol * 0.007874016f;
        ...
        for (i = 0; i < pssf->channels; i++) {
            ...
            pSourceVoice = &sndgs.chan[pVoice->voices[i]];
...
extern "C" int SNDPKTPLAY_submitspace(int packetinstancehandle) {
    SNDPACKETCHAN *ppchan;
...
extern "C" unsigned int SNDPKTPLAY_framesoutstanding(int packetinstancehandle) {
    SNDPACKETCHAN *ppchan;
...
extern "C" int SNDPKTPLAY_purge(int packetinstancehandle, int starthandle, int endhandle) {
...
void *SNDPKTPLAYI_get(int packetinstancehandle, int chan, int *pframes, int *pcontinuation) {
    ...
    if (releaseFlag != 0) {
        ...
        if (ppchan->preleasefunc != NULL) {
            ...
            pspcb->type = SNDI_PACKET_CB_RELEASE;
...
void SNDPKTPLAYI_freeframes(int packetinstancehandle, int chan, int frames) {
    ...
    if (ppchan->pframesfunc != NULL) {
        ...
        pspcb->type = SNDI_PACKET_CB_FREEFRAMES;
        pspcb->chan = packetinstancehandle;
...
void SNDPKTPLAYI_flushcallbackdata() {
    ...
    for (i = 0; i < sndpps.callbackindex; i++) {
        ...
        if (pspcb->type == SNDI_PACKET_CB_FREEFRAMES) {
