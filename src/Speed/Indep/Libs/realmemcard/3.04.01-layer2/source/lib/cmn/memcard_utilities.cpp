...
unsigned int Crc32(const void *buf, int bufsize) {
    ...
    d = static_cast<unsigned char *>(const_cast<void *>(buf));
    if (bufsize <= 3) {
        return 0;
    }

    result = static_cast<unsigned int>(*d) << 24;
    result |= static_cast<unsigned int>(*++d) << 16;
    result |= static_cast<unsigned int>(*++d) << 8;
    result = ~(result | *++d);
    d++;
    {
        int i;
...
static char *strrstr(char *str, char *searchfor) {
    char *best = nullptr;
    char *s;

    while (*searchfor != 0) {
        s = strrchr(str, *searchfor);
        if (s > best) {
            best = s;
        }
        searchfor++;
...
int Wildcard(char *nam, char *pat) {
    char *bop;
    int bopmatch;
    ...
    if (bop != nullptr) {
        ...
        if (*bop == '^') {
            *bop = 0;
            bopmatch = Wildcard(nam, pat) ^ Wildcard(nam, bop + 1);
            *bop = '^';
            return bopmatch;
        }
    }
