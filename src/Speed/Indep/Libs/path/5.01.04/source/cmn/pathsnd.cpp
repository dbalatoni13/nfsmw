...
#define PATH_AGG(x) ((*(__typeof__(x) (*)[1]) &(x))[0])

// The two entry points path uses that snd's public headers do not export; they
// live in snd's private sndcmn.h.
extern "C" {
...
int Path::PathToSnd::CreateStreamTrack(Path::IPathTrack **ppPathTrack, int maxRequests,
    ...
    if (bufferTime != 0.0f) {
        ...
        memsize = static_cast<int>(bufferTime * 1000.0f * byteRate);
...
Path::PathTrackSnd::PathTrackSnd() {
    ...
    PATH_AGG(Path::IPathToSnd::sndimp)->GetDefaultPlayOpts(this->mSndPlayOpts);
...
int Path::PathTrackSnd::GetPathStatus(PATHSTATUS *psps) {
    ...
    barlen = beatlen;
    int nodeIndex = this->mStatus.playingnode;
    if (nodeIndex >= 0 && nodeIndex <= Path::pfstate->pmap->numnodes) {
        nodeinfo = (PATHFINDNODE *)((int *)Path::pfstate->pmap +
        ...
        if (status >= 0) {
            ...
            if (timeleft < 0) {
                ...
                if (nodeinfo->extra.beat.playbeats != 0) {
                    {
                        // Written through a block temp on purpose: with
                        // `nodeinfo->beats - 1 - beat` GCC reassociates to
                        // `beats - (beat + 1)` and the decrement no longer
                        // reuses the register holding beats.
                        int b = nodeinfo->beats - 1;

                        remainingbeats = b - this->mRequest.beat;
                    }
...
void Path::PathTrackSndStream::CheckStatus() {
    ...
    if (this->mHandle >= 0) {
        ...
        this->mUpdateStatus = 0;
        if (PATH_AGG(Path::timercallsinarow) > 0 && this->mStatus.timeremaining > 0) {
            int correction;

            correction = PATH_AGG(Path::timercallsinarow) * Path::pfstate->timerinterval;
...
int Path::PathTrackSndStream::SetVolume(int volume) {
    ...
    if (this->PathTrackSnd::SetVolume(volume) == 0) {
        SNDSYS_entercritical();
        result = SNDSTRM_setvol(this->mHandle, static_cast<Snd::Channel>(-1), static_cast<float>(volume) / 127.0f);
        SNDSYS_leavecritical();
        this->mVolume = volume;
