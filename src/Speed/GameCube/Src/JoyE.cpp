            ...
            for (port = 0; port <= 3; port++) {
                ...
                if (HardwarePadStatus[port].err == PAD_ERR_NONE) {
                    ...
                    joy_data->ThePadData[0].DigitalButtons =
                        ~(((buttons >> 8) & 1) << 0 | ((buttons >> 9) & 1) << 1 | ((buttons >> 10) & 1) << 2 | ((buttons >> 11) & 1) << 3 |
                          ((buttons >> 4) & 1) << 4 | ((buttons >> 12) & 1) << 5 | ((buttons >> 3) & 1) << 8 | ((buttons >> 2) & 1) << 9 |
                          ((buttons >> 0) & 1) << 10 | ((buttons >> 1) & 1) << 11);
                    ...
                    if (plat_lgwheels->PedalsConnected(port)) {
                        joy_data->ThePadData[slot].AnalogRightY = reinterpret_cast<LGPosition *>(plat_lgwheels)[port].accelerator;
