...
inline void Realmc::GcFileHeader::Init(unsigned int userDataSize, unsigned int iplDataSize) {
    /* El objetivo evalua (userDataSize + 0x10) + iplDataSize: `addi` sobre el
       PRIMER operando escrito. Con el orden natural GCC 2.9 le suma el 0x10 a
       iplDataSize y OpenFile pierde dos instrucciones. */
    this->mFileSize = iplDataSize + 0x10 + userDataSize;
