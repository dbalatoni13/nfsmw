...
wchar_t sCardName[48];


void GCInterface::TrcStartGame(const StartGameInfo &info) {
    this->CheckMessageCompatibility();
    this->mMutex->Lock();
    TaskTrcStartGame *task = &GCInterface::mTaskTrcStartGame;

    task->Start(&info);
    task->mParent->StartTask(task);
    this->mMutex->Unlock();
}
...
void GCInterface::TrcLoadFile(const CardID &cardID, const FileInfo &fileInfo) {
    this->mMutex->Lock();
    TaskTrcLoadFile *task = &GCInterface::mTaskTrcLoadFile;

    task->Start(cardID, &fileInfo);
    task->mParent->StartTask(task);
    this->mMutex->Unlock();
}
...
void GCInterface::TrcSaveFile(const CardID &cardID, const FileInfo &fileInfo, SaveTaskType saveTaskType,
    ...
    this->mMutex->Lock();
    TaskTrcSaveFile *task = &GCInterface::mTaskTrcSaveFile;

    task->Start(cardID, &fileInfo, saveTaskType, nBlocksNeeded, nFilesNeeded);
    task->mParent->StartTask(task);
    this->mMutex->Unlock();
}
...
void GCInterface::TrcListFiles(const CardID &cardID, const FileInfo &fileInfo, ListTaskType) {
    this->mMutex->Lock();
    TaskTrcListFiles *task = &GCInterface::mTaskTrcListFiles;
...
void GCInterface::TrcDeleteFile(const CardID &cardID, const FileInfo &fileInfo) {
    this->mMutex->Lock();
    TaskTrcDeleteFile *task = &GCInterface::mTaskTrcDeleteFile;
...
const unsigned short *GCInterface::GetCardName(const CardID &cardID, wchar_t *namebuf, int buflen) {
    ...
    *reinterpret_cast<unsigned int *>(slot) = 0;
    ...
    p = reinterpret_cast<const unsigned short *>(Locale::GetString(0x28, "s", slot));
