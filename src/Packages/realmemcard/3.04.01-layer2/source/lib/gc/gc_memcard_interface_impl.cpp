#define REALMC_GC_MESSAGE_DTOR_OUTOFLINE
#include <string.h>
...
/* DEUDA r27: el literal "*" tiene que quedar internado como el 6o de la TU
   (va detras de las cinco cadenas LC_msg de gc_interface_impl.h y delante del
   "" de SetAutosave). La definicion de ALL_ENTRIES esta al final del fichero
   porque su .sdata va DETRAS de slotA/slotB de _LcGetSlotString. */
...
inline void GCMessage::Init() {
    this->Clear();
}

inline void GCMessage::_SetMsgOptions(int options) {
    int curOption;
    int iOption;
...
inline short *GCMessage::_LcGetSlotString(int slotnum) {
