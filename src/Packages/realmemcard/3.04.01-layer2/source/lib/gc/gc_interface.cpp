...
#define REALMC_GC_MESSAGE_INIT_INLINE
#define REALMC_BLOCKCALC_NO_INTERFACE
/* GCMSG_INTERFACE_ONLY enciende un `#pragma interface` acotado a `GCMessage`
   dentro de gc_interface_impl.h: se cierra con un `#pragma implementation`
   puesto justo detras de la clase, que restaura el estado para `GCInterface`
   --comprobado--. El nombre que lleva ese pragma da igual, y con uno que no
   sea el del propio fichero GCC no avisa de `appears after its #include`. Sin el, la unidad DEFINE `_vt.GCMessage` (24 B),
   `_._GCMessage` (52 B), `Init` (32 B) y `Clear` (40 B), que el objetivo
   referencia como UNDEF porque viven en gc_memcard_interface_impl.o. Definir
   `REALMC_GC_MESSAGE_DTOR_OUTOFLINE` NO vale: el objetivo no llama al
   destructor en ningun sitio (`__tcf_0` solo repone el vptr).

   TASK_INLINES_IN_TU saca de la clase los cuerpos de `TaskManager::
   UpdateCurrentTask` y `TaskTrc::Init`, que esta unidad define al final, detras
   de `EndTask`: es donde el objetivo los tiene. */
#define REALMC_GCMSG_INTERFACE_ONLY
#define REALMC_TASK_INLINES_IN_TU
#include <stdio.h>
#include <string.h>
...
const char *ROOT_DIRECTORY_NAME = 0;
const char *FILENAME_ALL_FILES = "*";
...
InputOptions GCInterface::ConvertUmsgToOption(UserMessage umsg, int options, TaskID taskID) {
    ...
    sprintf(errorMsg, "Error in GCInterface::ConvertUmsgToOption: received invalid user message, %d\n", umsg);
    return IO_NONE;
}
...
inline void TaskManager::UpdateCurrentTask() {}

inline void TaskTrc::Init(bool nofifyUser) {
    this->Clear();
