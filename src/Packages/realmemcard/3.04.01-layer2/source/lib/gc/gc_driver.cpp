...
ICardResult GCDriver::OpenFile(const CardID &cardID, const FileInfo *pFileInfo, FileOpenMode fmode, OpenFileDescriptor **handle) {
    ...
    *handle = nullptr;
    ...
    if (convertNGCerror) {
        result = this->ConvertCardResult(coResult);
    }
    ...
    CARDGetStatus(fd->mCardID.slot, fd->mGcFileInfo.fileNo, &fd->mGcCardStat);
    *handle = fd;
...
int GCDriver::SetHeaderInfo(GcFileDescriptor *fd) {
    ...
    if (fd->mFileInfo.gcIconDataInfo->animationLoop == GCIL_BACK_AND_FORTH) {
        ...
        fd->mGcCardStat.bannerFormat = (u8)((fd->mGcCardStat.bannerFormat & ~4) | 0);
    }
...
ICardResult GCDriver::ReadFile(OpenFileDescriptor *handle, void *pBuffer, int bytesToRead, int *nBytesRead) {
    ...
    do {
        ...
        if (fd->readNextSector) {
            ...
            unsigned int sectorSize = this->mSectorSize;
            long fileSectorOffset = fd->mFileSectorOffset;
            fd->readNextSector = result;
            fd->mFileSectorOffset = fileSectorOffset + sectorSize;
        }
...
void GCDriver::RecordIplDataChecksum(GcFileDescriptor *fd) {
    ...
    for (iByte = 0, curByte = pFileInfo->comment1; iByte < pFileInfo->sizeofcomment1; iByte++) {
        checksum += *curByte;
        curByte++;
    }

    for (iByte = 0, curByte = pFileInfo->comment2; iByte < pFileInfo->sizeofcomment2; iByte++) {
        checksum += *curByte;
        curByte++;
    }
...
ICardResult GCDriver::VerifyIplDataChecksum(GcFileDescriptor *fd) {
    ...
    while (bytesLeftToRead > 0) {
        if (bytesLeftToRead < (int)GCDriver::MEMCARD_SECTOR_SIZE) {
            readBlockSize = bytesLeftToRead;
        }
        result = this->ReadFile(fd, 0, readBlockSize, 0);
        if (result != CR_SUCCESS) {
            break;
        }
        ...
        for (iByte = readStartPosition; iByte < readEndPosition; iByte++) {
