/*-========================================================================-_
 ...
 |PARENT:   N / A                       MINREQ:  Win2000, Xbox 360          |
 ...
 ^~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~^
  NOTES:
    ...
    2.  Volume level is expressed as a linear amplitude scaler:
        ...
        Amplification (volume > 1.0f) is also allowed, and is not clipped.
...
//--------------<D-E-F-I-N-I-T-I-O-N-S>-------------------------------------//
    ...
    #if defined(_XBOX)
        #define SPEAKER_XENON 0x3f 
        #define SPEAKER_XBOX  0x3f // Xbox 360 speaker geometry configuration, used with X3DAudioInitialize
    #endif
...
//--------------<D-A-T-A---T-Y-P-E-S>---------------------------------------//
    ...
    typedef struct X3DAUDIO_EMITTER
    {
        ...
        X3DAUDIO_VECTOR OrientFront; // orientation of front direction, used only with multi-channel emitters for matrix and delay calculations or single-channel emitters with cones for matrix, LPF (both direct and reverb paths), and reverb calculations, must be normalized when used
        X3DAUDIO_VECTOR OrientTop;   // orientation of top direction, used only with multi-channel emitters for matrix and delay calculations, must be orthonormal with OrientFront when used

        X3DAUDIO_VECTOR Position; // position in user-defined world units, does not affect Velocity
        X3DAUDIO_VECTOR Velocity; // velocity vector in user-defined world units/second, used only for doppler calculations, does not affect Position

        UINT32 ChannelCount;       // number of sound channels, must be > 0
        FLOAT32 ChannelRadius;     // channel radius, used only with multi-channel emitters for matrix and delay calculations, must be >= 0.0f when used
        FLOAT32* pChannelAzimuths; // channel azimuth array, used only with multi-channel emitters for matrix and delay calculations, contains positions of each channel expressed in radians along the channel radius with respect to the front orientation vector in the plane orthogonal to the top orientation vector, or X3DAUDIO_2PI to specify a LFE channel, must have at least ChannelCount elements, all within [0.0f, X3DAUDIO_2PI] when used
...
//--------------<F-U-N-C-T-I-O-N-S>-----------------------------------------//
    ...
    STDAPI_(void) X3DAudioInitialize (UINT32 SpeakerChannelMask, FLOAT32 SpeedOfSound);

    // calculates DSP settings with respect to 3D parameters
    STDAPI_(void) X3DAudioCalculate (const X3DAUDIO_LISTENER* pListener, const X3DAUDIO_EMITTER* pEmitter, UINT32 Flags, X3DAUDIO_DSP_SETTINGS* pDSPSettings);
